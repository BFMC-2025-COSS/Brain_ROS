...
class Window(object):
    ...
    def __init__(self, y1, y2, x, m=40, tolerance=50):
        """
    ...
    def pixels_in(self, nonzero, x=None):
        ...
        Notes
        -----
        `nonzero` contains two arrays of coordinates of non-zero pixels. Say, there were 50 non-zero pixels in the image and `nonzero` would contain two arrays of
        ...
        if len(win_indices) > self.tolerance:
            new_x = np.int32(np.mean(nonzero[1][win_indices]))
            self.x = new_x
            self.mean_x = new_x
        else:
            self.mean_x = self.x
